...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    unsample_layer7_out = tf.layers.conv2d_transpose(layer7_conv1x1,num_classes,4,2,padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    layer4_conv1x1 = tf.layers.conv2d(vgg_layer4_out,num_classes,1,padding='same',
                               kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    unsample_layer4_in = tf.add(unsample_layer7_out,layer4_conv1x1);
    unsample_layer4_out = tf.layers.conv2d_transpose(unsample_layer4_in,num_classes,4,2,padding='same',
                                           kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    layer3_conv1x1 = tf.layers.conv2d(vgg_layer3_out,num_classes,1,padding='same',
                                      kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    unsample_layer3_in = tf.add(unsample_layer4_out,layer3_conv1x1);
    unsample_layer3_out = tf.layers.conv2d_transpose(unsample_layer3_in,num_classes,16,8,padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))

    return unsample_layer3_out
...
def run():
    ...
    with tf.Session() as sess:
        ...
        epochs = 45    # 5 15 45, try different epoch size 
        batch_size = 5
